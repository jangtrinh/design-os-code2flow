...
export function frameNode(id: string, x: number, y: number, opts: FrameOpts, onSelect: SelectFn, onOpenStory: OpenStoryFn): SVGGElement {
  ...
  const kindIcon = s.kind === "route" ? "app-window" : s.kind === "tab" ? "tabs" : /wizard/i.test(s.kind) ? "list-numbers" : "cards";
  head.append(iconSvg(kindIcon, 0, 0, 14), el("text", { x: 16, y: 9, class: "eyebrow" }, fitText(featureTitle.toUpperCase(), (w - 2 * PAD) * 0.55, 10 * 0.62)));
  ...
  if (!isState) {
    const st = opts.stats ?? { inplace: 0, intra: 0, dismiss: 0, sinks: {}, low: 0 }; const chips: [string, string, Parameters<typeof iconSvg>[0]][] = [];
    const kids = D.graph.screens.filter((z) => z.parentScreenId === id).length;
    if (dd.shot.clipped) chips.push([`continues ${Math.round(D.meta[id].height)} px tall`, "", "arrow-right"]);
    if (shellTargets.has(id)) chips.push(["in sidebar", "", "sidebar-simple"]);
    if (kids) chips.push([`${kids} states`, "", "cards"]); if (st.inplace) chips.push([`in-place ${st.inplace}`, "", "cursor-click"]); if (st.dismiss) chips.push([`closes ${st.dismiss}`, "", "arrow-u-up-left"]);
    for (const [k, v] of Object.entries(st.sinks)) chips.push([`to ${k} ×${v}`, "", "link-break"]); if (st.low) chips.push([`review ${st.low}`, "warn", "warning"]);
    const chipG = uiScale("ui-scale", PAD, HEAD + sh + PAD - 2); let cx = 0;
    for (const [t, c, i] of chips) { const cw = t.length * 6 + 26; if (cx + cw > w - 2 * PAD) break; chipG.append(el("rect", { x: cx, y: 0, width: cw, height: 18, rx: 9, class: "chipbg " + c }), iconSvg(i, cx + 5, 3, 12), el("text", { x: cx + 20, y: 13, class: "chip " + c }, t)); cx += cw + 6; }
    g.append(chipG);
  }
