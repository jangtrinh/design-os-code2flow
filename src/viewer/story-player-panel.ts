...
export function renderPlayerPanel(panel: HTMLElement, story: Story, index: number, h: PlayerPanelHandlers): void {
  ...
  if (step.via || edge) { const evidence = document.createElement("button"); evidence.className = "player-evidence"; evidence.type = "button"; const trigger = step.via ?? edge?.trigger ?? ""; const meta = `${edge?.confidence ?? "asserted"} · ${edge ? `${edge.evidence.file}:${edge.evidence.line}` : "not in code"}`; evidence.title = `via ${trigger} · ${meta}`; evidence.innerHTML = `<span class="player-evidence-trigger">${iconHtml("cursor-click", 16)}<span>via ${trigger}</span></span><span class="player-evidence-meta">${meta}</span>`; panel.append(evidence);
  ...
  if (story.branches?.length) { const branches = document.createElement("div"); branches.className = "player-branches"; story.branches.forEach((branch) => { const pill = document.createElement("span"); pill.className = "player-branch player-badge"; pill.innerHTML = `${iconHtml("git-branch", 14)}<span>${branch.title}</span>`; branches.append(pill); }); panel.append(branches); }
  if (!screen || !edge && index > 0) add("p", "player-warning", screen ? "Not in code" : "Missing screen");
  const actions = document.createElement("div"); actions.className = "player-actions"; const canvas = document.createElement("button"); canvas.type = "button"; canvas.innerHTML = `${iconHtml("arrow-square-out")}<span>Open canvas</span>`; canvas.addEventListener("click", h.canvas); actions.append(canvas);
  if (screen) { const shot = document.createElement("button"); shot.type = "button"; shot.innerHTML = `${iconHtml("image")}<span>Screenshot</span>`; shot.addEventListener("click", () => h.screenshot(screen)); actions.append(shot); } panel.append(actions);
}
