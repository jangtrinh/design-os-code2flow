...
import { byId, D, featById, realTitle, routeOf, routeTitle, storyFeature, storyPath, escapeHtml as esc } from "./data-model.js";
...
export function renderPlayerPanel(panel: HTMLElement, story: Story, index: number, focus: boolean, h: PlayerPanelHandlers): void {
  ...
  if (step.via || edge) { const evidence = document.createElement("button"); evidence.className = "player-evidence-label"; evidence.type = "button"; const trigger = step.via ?? edge?.trigger ?? ""; const meta = edge ? `${edge.evidence.file}:${edge.evidence.line}` : "Not in code"; evidence.title = edge?.confidence ?? "missing"; evidence.innerHTML = `<span class="player-evidence-trigger">${iconHtml("cursor-click", "Action trigger", 16)}<span>${esc(trigger)}</span><span class="confidence-dot" title="${edge?.confidence ?? "missing"}">${edge ? (edge.confidence === "high" ? "●" : edge.confidence === "medium" ? "◐" : "○") : iconHtml("warning", "Missing screen", 14)}</span></span><span class="player-evidence-meta-label">${iconHtml(edge ? "file-code" : "link-break", edge ? "Source evidence" : "Missing screen", 14)}${esc(meta)}</span>`; panel.append(evidence);
