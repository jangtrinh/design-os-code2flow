...
import { D, byId, featById, featureOf, realTitle, routeOf, routeTitle, shellTargets } from "./data-model.js";
...
function evidenceRow(edge: Bundle["primary"], group: Bundle["primary"][] = [edge]): HTMLButtonElement {
  ...
  row.innerHTML = `<span class="confidence-dot" title="${edge.confidence}">${confidenceIcon(edge.confidence)}</span>${iconHtml(triggerIcon(edge), "Trigger", 16)}<span class="inspector-trigger">${edge.trigger}</span>${iconHtml("caret-right", "From to", 14)}<span class="inspector-target">${targetTitle(edge.target)}</span>${count}`;
  const evidence = document.createElement("pre"); evidence.className = "inspector-evidence"; evidence.hidden = true;
  evidence.innerHTML = group.map((e) => `${iconHtml("file-code", "Source evidence", 14)}<span>${e.evidence.file}:${e.evidence.line}</span>${e.evidence.snippet ? `<code>${e.evidence.snippet}</code>` : ""}`).join("\n");
  row.addEventListener("click", () => { evidence.hidden = !evidence.hidden; }); row.after(evidence); return row;
}
