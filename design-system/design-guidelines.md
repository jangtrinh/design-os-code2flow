...
- **Icons (decided 2026-09-04): Phosphor, regular weight, everywhere.** Generated offline into `src/viewer/icons.ts` by `npm run build:icons` (`@phosphor-icons/core`, MIT). Every control, panel heading, list row and chip carries an icon; text alone is not enough. 16 px in chrome (20 px in list rows), `currentColor`, 1 px optical gap to its label. Mapping: product map `map-trifold` · feature `squares-four` · story `path` · route `app-window` · modal/drawer `cards` · tab `tabs` · wizard step `list-numbers` · shell nav `sidebar-simple` · back `caret-left` · search `magnifying-glass` · Inspect `eye` · Present `presentation` · Play `play` · zoom `plus`/`minus` · fit `corners-out` · prev/next `caret-left`/`caret-right` · close `x` · selected `check` · dropdown `caret-down` · evidence `file-code` · low confidence `warning` · broken/missing `link-break` · Cancel/Close edges `arrow-u-up-left` · screenshot `image` · open in canvas `arrow-square-out` · branch `git-branch` · entry `sign-in` · exit `sign-out` · form `textbox` · button `cursor-click` · external `arrow-square-out` · keyboard hint `keyboard`.

## Icons

- **Less at a distance.** Frames drop detail as the canvas zooms out (lod-0 < 0.35: screenshot + title; lod-1 < 0.7: + kind icon and edge pills; lod-2: everything). Text that cannot be read at the current scale is not drawn.
- **Overlays are frames of their own.** Every modal, dropdown, tooltip, popover and drawer is a State Screen with its own frame next to its parent route, on a tinted container (shade-100, border shade-300) with a kind eyebrow, never inferred from the parent screenshot.
- **Nothing overlays the screenshot.** Badges (Entry, Exit, story counts, step numbers, selection markers) live in the frame header or footer chip row; no chrome is ever drawn inside the screenshot rect.
- **Every icon explains itself on hover.** Every icon in the viewer (canvas, pills, chips, rail, toolbar, panels, legend, player) carries a `title` and `aria-label` naming its meaning; counted chips read `<meaning> · <n>`. An icon without a label is a lint failure in the viewer browser test.
